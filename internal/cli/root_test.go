...
func TestLiveArchiveLoopArchivesSafetyWindowedHead(t *testing.T) {
	...
	errCh := startLiveArchiveLoop(ctx, reader, objectStore, archive.LiveArchiveOptions{
	...
	select {
	case loopErr := <-errCh:
		t.Fatalf("archive loop failed: %v", loopErr)
	...
	if verify.BlocksChecked != 3 {
		t.Fatalf("verified %d archived blocks, want 3", verify.BlocksChecked)
	}
}

func TestLivePruneLoopPrunesVerifiedArchive(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	dbDir := filepath.Join(t.TempDir(), "db")
	...
	errCh := startLivePruneLoop(ctx, reader, objectStore, archive.PruneHotOptions{
	...
	for reader.Base() != 5 {
		select {
		case loopErr := <-errCh:
			t.Fatalf("prune loop failed: %v", loopErr)
		case <-deadline:
			t.Fatalf("base after prune = %d, want 5", reader.Base())
		default:
			time.Sleep(10 * time.Millisecond)
		}
	}
	if reader.LoadBlock(4) != nil || reader.LoadBlock(5) == nil {
		t.Fatalf("unexpected retained range: base=%d height=%d", reader.Base(), reader.Height())
	}
}

func TestLivePruneLoopRejectsCorruptArchive(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	dbDir := filepath.Join(t.TempDir(), "db")
	createCLIBlockStoreFixture(t, dbDir, 4)
	...
	case <-time.After(500 * time.Millisecond):
		t.Fatal("expected prune loop to fail on corrupt archive")
	}
	if reader.Base() != 1 {
...
func TestLiveMaintenanceLoopArchivesThenPrunes(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	dbDir := filepath.Join(t.TempDir(), "db")
	createCLIBlockStoreFixture(t, dbDir, 5)
	...
	manifestKey := archive.ManifestKey("archive", cliTestChainID, archive.DefaultManifest)
	errCh := startLiveMaintenanceLoop(ctx, reader, objectStore, archive.LiveArchiveOptions{
		ChainID:       cliTestChainID,
		Prefix:        "archive",
		SegmentBlocks: 2,
		Compression:   archive.CompressionGzip,
